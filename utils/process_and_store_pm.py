...
# Internal helpers
# -----------------------------------------------------------------------------
class Vectortimegenerator:
    def __init__(self):
        self.engine = create_engine(DB_URL, pool_size=5, max_overflow=10, pool_timeout=30)
        logger.info("Database engine created")

    def _load_dataframe(self, table_name: str, variable: str, run_date: _date) -> pd.DataFrame:
    ...
    def generate_and_store_vector_tiles(
        ...
        Args:
            ...
            zoom_min: Minimum zoom level for tiles (default: 2).
        ...
        df = self._load_dataframe(table_name, variable, run_date)
        if df.empty:
        ...
        df[variable] = df[variable].round().astype(int)  # Scale for MVT storage efficiency
        ...
        target_table = f"{table_name}_tiles"  # Using '_tiles' for MVT convention
    ...
    def generate_pm25_vector_tiles(self) -> None:
        """Generate tiles for PM2.5 (cams_pm25 table)."""
        self.generate_and_store_vector_tiles(variable="pm2p5", table_name="cams_pm25")
        logger.info("PM2.5 tiles stored")

    def generate_pm10_vector_tiles(self) -> None:
        """Generate tiles for PM10 (cams_pm10 table)."""
        self.generate_and_store_vector_tiles(variable="pm10", table_name="cams_pm10")
        logger.info("PM10 tiles stored")


if __name__ == "__main__":
    vtgen = Vectortimegenerator()
    vtgen.generate_pm25_vector_tiles()
    vtgen.generate_pm10_vector_tiles()
